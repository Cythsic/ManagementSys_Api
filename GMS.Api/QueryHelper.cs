    ...
    public static class QueryHelper
    {
        ...
        public static string GetWhereStr(this HttpContext context, Dictionary<string, object> query)
        {
            ...
            var parms = keys.Where(p => (p.Contains("SL_")
                || p.Contains("SEB_")) || p.Contains("SES_") || p.Contains("SEGT_")
                || p.Contains("SELT_") || p.Contains("SEI_") || p.Contains("SENE_")
                || p.Contains("SLL_") || p.Contains("SLR_"));

            var parms2 = keys.Where(p => !p.Contains("sort") && !p.Contains("order") && !p.Contains("pageNumber") && !p.Contains("pageSize"));

            foreach (var parm in parms2)
            {
                var name = parm.Split('_');
                var keyPosition = name[0].Length + 1;

                var fieldName = "";
                if (parm.Split('_').Length == 1)
                    fieldName = name[0];
                else fieldName = parm.Substring(keyPosition, parm.Length - keyPosition);
                ...
                switch (name[0])
                {
                    ...
                    default:
                        strWhere += string.Format(" And {0} like '%{1}%' ", fieldName, value);
                        break;
                }
            }
